...
if submit:
      if supply_wo_margin == 0.0 or margin == 0.0:
        st.warning("⚠️ Please enter non-zero values for margin and supply to get a meaningful prediction.")
      else:
        input_data = {
            'Substance': substance,
            'Unit': unit,
        ...
        st.success(f"Predicted Supply Chain Emission Factor with Margin: **{prediction[0]:.4f}** kg CO2e/2018 USD")

        # Simple bar chart
        st.markdown("### Emission Factor Visualization")
        chart_df = pd.DataFrame({'Predicted Emission Factor': [prediction[0]]})
